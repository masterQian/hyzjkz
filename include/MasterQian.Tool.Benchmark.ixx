...
namespace MasterQian::Tool {
	...
	/// <para>参数列表是一个std::vector, 你可以方便地使用初始化列表{ }构造</para>
	...
	export struct Benchmark {
		...
		template<typename Arg>
		Benchmark& Add(Function<Arg> func, std::vector<Arg> const& args, mqui64 count = 1ULL, std::wstring_view name = L"") noexcept {
			details::MasterQian_Tool_Benchmark_BenchmarkAdd(func, args.data(), args.size(), sizeof(Arg), count, name.data());
			return *this;
		}
		...
		Benchmark& Add(Function<nullarg> func, mqui64 count = 1ULL, std::wstring_view name = L"") noexcept {
			return Add<nullarg>(func, { { } }, count, name);
		}
